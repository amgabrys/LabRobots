...
def run(ctx):
    """
    Modified protocol with thermocycler removed, original positions preserved
    """
    trash_chute    = True  # Changed from False
    USE_GRIPPER    = True
    dry_run        = False
    mount          = 'right'
    ...
    # Load tips - keeping original positions
    ...
    tips = [*tips1000.wells(),*tips1001.wells(),*tips1002.wells()]
    
    # Load instruments
    m1000 = ctx.load_instrument('flex_8channel_1000', mount, tip_racks=[tips1000, tips1001, tips1002])
    ...
    def tiptrack(pip, tipbox):
    	...
    	# Removed the pause for emptying trash since we're using a trash chute
    	# if drop_count >= 150:
    	#     drop_count = 0
