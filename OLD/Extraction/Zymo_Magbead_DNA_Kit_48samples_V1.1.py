...
def run(ctx):
    ...
    def bind(vol1,vol2):
        ...
        for i, well in enumerate(samples_m):
            ...
            source = binding_buffer[(i // 2) % len(binding_buffer)]
            if i == 0:
                reps=5
            else:
                reps=2
        ...
        for i, well in enumerate(samples_m):
            ...
            source = bind2_res[(i // 3) % len(bind2_res)]
            if i == 0 or i == 3:
                height = 10
            else:
                height = 1
    ...
    def wash(vol, source):
        ...
        const = max(1, num_cols // len(active_wells))  # <-- prevent zero division
