...
metadata = {
    'protocolName': 'Zymo Magbead DNA Extraction 48 Samples V1',
    'author': 'OpentronsAI',
...
def run(ctx):
    ...
    # Load other modules and labware
    ...
    # Load tips
    tips1000 = ctx.load_labware('opentrons_flex_96_tiprack_1000ul', 'A2','Tips 1')
    tips1001 = ctx.load_labware('opentrons_flex_96_tiprack_1000ul', 'B2','Tips 2')
    tips1002 = ctx.load_labware('opentrons_flex_96_tiprack_1000ul', 'B3','Tips 3')
    tips1003 = ctx.load_labware('opentrons_flex_96_tiprack_1000ul', 'A1','Tips 4')
    tips1004 = ctx.load_labware('opentrons_flex_96_tiprack_1000ul', 'B1','Tips 5')
    ...
    # The ranges are because adjusting sample volume should change how many wells are used.
    ...
    colors = ['#008000','#008000','#FF00FF','#A52A2A','#00FFFF','#00FFFF','#800080',
    ...
    liquids = ['MagBinding Beads','MagBinding Buffer','MagBinding Buffer (used as wash)','MagWash 1','MagWash 2 (1)','MagWash 2 (2)','DNase/RNase Free Water']
    ...
    def liquids_(liq_name, location, color, vol):
        sampnum = 8*(math.ceil(num_samples/8))

        if liq_name == "MagBinding Beads":
            extra_samples = math.ceil(1500/bind_vol)
        else:
            extra_samples = math.ceil(1500/vol)

        if isinstance(location, list):
            # Account for dead volume at bottom of reservoir wells
            dead_volume = 1500  # µL - volume that cannot be aspirated
            max_usable_volume = 13500 - dead_volume  # 12,000 µL usable
        
            # Calculate max samples per well based on usable volume
            max_samples_per_well = int(max_usable_volume / vol)
        
            # Calculate minimum number of wells needed
            wells_needed = math.ceil(sampnum / max_samples_per_well)
        
            # Distribute samples across minimum wells
            samples_per_well = []
            remaining_samples = sampnum
        
            for j in range(wells_needed):  # Changed 'i' to 'j' to avoid conflict
                if remaining_samples > max_samples_per_well:
                    samples_per_well.append(max_samples_per_well)
                    remaining_samples -= max_samples_per_well
                else:
                    samples_per_well.append(remaining_samples)
                    remaining_samples = 0
        
            liquid_obj = ctx.define_liquid(name=str(liq_name), description=str(liq_name), display_color=color)
            for sample, well in zip(samples_per_well, location[:len(samples_per_well)]):
                v = vol*(sample+extra_samples) + dead_volume  # Add dead volume to total
                well.load_liquid(liquid=liquid_obj, volume=v)
        else:
            v = vol*(sampnum+extra_samples) + 1500  # Add dead volume for single well
            liquid_obj = ctx.define_liquid(name=str(liq_name), description=str(liq_name), display_color=color)
            location.load_liquid(liquid=liquid_obj, volume=v)
    ...
    def wash(vol, source):
        ...
        # Calculate how many wells are actually needed based on volume optimization
        dead_volume = 1500
        max_usable_volume = 13500 - dead_volume
        max_samples_per_well = int(max_usable_volume / vol)
        wells_with_liquid = math.ceil(num_samples / max_samples_per_well)
        
        # Use only the wells that have liquid
        active_wells = source[:wells_with_liquid]
        const = num_cols // len(active_wells)
        ...
        tiptrack(m1000,tips)
        for i, m in enumerate(samples_m):
            # Determine aspiration height
            if source == wash1:
                if i == 0 or i == (num_cols // 2):
                    height = 10
                else:
                    height = 1
            else:
            ...
            # Select the appropriate well based on column distribution
            src = active_wells[i // const]
