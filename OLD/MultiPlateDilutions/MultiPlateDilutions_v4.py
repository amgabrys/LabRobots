...
metadata = {
    ...
    'author': 'OpentronsAI',
    'description': 'Dilute samples from multiple plates with automatic volume splitting',
    'source': 'OpentronsAI'
}
...
def run(protocol: protocol_api.ProtocolContext):
    ...
    # Large dilutions: 1 tip for all water + 1 tip per dilution (sample + mix + 50µL transfer)
    if wells_large_dilution:
        total_tips_needed += 1  # One tip for all water
        total_tips_needed += len(wells_large_dilution)  # One tip per dilution (sample + mix + transfer)
    
    # Water first: 1 tip for all water additions + 1 tip per sample
    ...
    if wells_large_dilution:
        ...
        # Pick up ONE tip for all water additions
        ...
        for idx, row in enumerate(wells_large_dilution):
            ...
            protocol.comment(f"Adding water to dilution {idx + 1}/{len(wells_large_dilution)}")
        ...
        # Drop the tip after all water additions
        p20_single.drop_tip()
