...
# Make it such that a counterweight is made at each step using water in a different reservior
# Probably make it such that adding the buffer stops after 

metadata = {
    'protocolName': 'ZR-96 DNA Clean & Concentrator-5 Protocol V7',
    'author': 'OpentronsAI',
...
def run(protocol: protocol_api.ProtocolContext):
    ...
    # First 150 µL wash
    for i, column in enumerate(deep_well_plate.columns()[:3]):
        wash_source = wash_buffer_wells[i % len(wash_buffer_wells)]
        p300_multi.aspirate(150, wash_source)
        p300_multi.dispense(150, column[0])
        protocol.delay(seconds=1)  # Let solution settle
    ...
    # Pause for centrifugation
    protocol.comment("You must add the 300 ul to each well in columns 4-12 by hand!")
    protocol.pause("Remove the deep well plate, centrifuge, and place back on the OT-2. Resume when ready.")
